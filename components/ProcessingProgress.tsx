    ...
    <div className={className}>
      ...
      {errorCount > 0 && (
        <div className="mt-6 p-4 bg-wine-red/10 border border-wine-red/40 rounded-xl">
          ...
          {/* NOTE: Show individual error messages for each failed item */}
          <div className="space-y-2 mt-3 pt-3 border-t border-wine-red/20">
            {items
              ...
              .map(item => (
                <div key={item.id} className="flex items-start gap-2">
                  <div className="w-8 h-8 rounded overflow-hidden flex-shrink-0">
                    {item.preview && (
                      <img src={item.preview} className="w-full h-full object-cover opacity-50 grayscale" alt="" />
                    )}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="font-body text-xs text-wine-cream/70 truncate">
                      {item.file.name}
                    </p>
                    <p className="font-body text-xs text-wine-red/90 mt-0.5">
